...
fetch('/cart.js')
    .then(res => res.json())
    .then(cart => {
        window.ShopifyCart.data = cart;
        window.ShopifyCart.ready = true;

        document.dispatchEvent(
            new CustomEvent('cart:ready', { detail: cart })
        );
    });
...
class ProductForm extends HTMLFormElement {
    ...
    handleFormSubmit = (event) => {
        ...
        // if (FoxTheme.settings.cartType === 'page_ajax') {
        //     console.log("Page Ajaxination On..")
        // };
        ...
        const addQty = this.formData.get("quantity") || 1;
        this.handleFormSubmission(config, addQty);
    };
    ...
    handleFormSubmission = (config, addQty) => {
        ...
        fetch(`${FoxTheme.routes.cart_add_url}`, config)
            ...
            .then(async (parsedState) => {
                ...
                // console.log("COOCOCOCOCOOC : :", addQty)
                if (cartCount) {
                    let getCount = parseInt(cartCount.innerText);
                    let newCount = getCount + parseInt(addQty);
                    cartCount.removeAttribute('hidden');
                    cartCount.innerText = newCount;
                }

                let productCard = this.closest('.product-card__wrapper');
                if (productCard) {
                    productCard.querySelector('.product-item-cart-stepper-wrapper').style.display = "block";
                    productCard.querySelector('.product-item-cart-stepper-wrapper').querySelector('.cart-stepper-ui-input').value = 1;
                    this.querySelector('.product-card__atc').style.display = 'none';
                }
                ...
                window.ShopifyCart.data = cartJson;
                document.dispatchEvent(
                    new CustomEvent('cart:updated', { detail: cartJson })
                );
    ...
    updateCartState = (cartJson) => {
        // console.log("Updating Cart States ")
...
class CardQty extends HTMLElement {
    constructor() {
        super();

        this.variantId = Number(this.dataset.variantId);
        this.input = this.querySelector('.cart-stepper-ui-input');
        this.plusBtn = this.querySelector('.cart-stepper-ui-plus');
        this.minusBtn = this.querySelector('.cart-stepper-ui-minus');
        this.pCard = this.closest('.product-card__wrapper');
    ...
    connectedCallback() {
        this.plusBtn?.addEventListener('click', () => this.changeQtyDebounced(1));
        this.minusBtn?.addEventListener('click', () => this.changeQtyDebounced(-1));

        if (window.ShopifyCart?.ready) {
            this.syncQtyFromCart(window.ShopifyCart.data);
        }
        document.addEventListener('cart:ready', this.onCartReady);
    ...
    disconnectedCallback() {
        ...
        document.removeEventListener('cart:ready', this.onCartReady);
        document.removeEventListener('cart:updated', this.onCartUpdated);
    }

    onCartReady(e) {
        this.syncQtyFromCart(e.detail);
    }
    ...
    syncQtyFromCart(cart) {
        // console.log("Getting Cart Data :::", cart)
        ...
        if (cartItems) {
            ...
            if (qty > 0) {
                this.style.display = '';
                this.pCard.querySelector('.product-card__atc').style.display = 'none'
            } else {
                this.style.display = 'none';
                this.pCard.querySelector('.product-card__atc').style.display = 'flex'
            }
        }
    }
    ...
    changeQtyDebounced(delta) {

        console.log("p delta before", this.pendingDelta)
        this.pendingDelta += delta;

        console.log("p delta aftre", this.pendingDelta)

        const cart = window.ShopifyCart?.data;
        if (cart) {
            const currentQty = this.getTotalQty(cart);
            const projectedQty = currentQty + this.pendingDelta;
            if (projectedQty >= 0) {
                this.input.value = projectedQty;
            } else {
                this.pendingDelta = -currentQty;
                this.input.value = 0;
            }
        }
        ...
        this.executeQtyChange();
        // this.debounceTimer = setTimeout(() => {
        // }, this.DEBOUNCE_DELAY);
    }

    async executeQtyChange() {
        if (this.isLoading || this.pendingDelta === 0) return;

        const cart = window.ShopifyCart?.data;
        if (!cart) return;

        const currentQty = this.getTotalQty(cart);
        const newQty = currentQty + this.pendingDelta;
        ...
        const deltaToApply = this.pendingDelta;
        this.pendingDelta = 0;

        this.setLoading(true);

        try {
            let changeBody = this.getFirstLineByVariant(cart, this.variantId);
            changeBody.quantity = changeBody.quantity + deltaToApply;

            this.handleCartChange(changeBody);

        } catch (err) {
            console.error('Cart update failed', err);
            this.syncQtyFromCart(cart);
        } finally {
            this.setLoading(false);
        }
    }

    getFirstLineByVariant(cartJSON, variantId) {
        const index = cartJSON.items.findIndex(
            item => item.variant_id === variantId
        );

        // if (index === -1) return null;

        console.log("Get Line for Var : ", variantId, "from Lines : ", cartJSON.items, "Line is : ", index + 1)
        return {
            line: index + 1,
            quantity: cartJSON.items[index].quantity
        };
    }

    handleCartChange = (changeBody) => {

        this.style.pointerEvents = 'none';
        this.querySelector('.product-item-cart-stepper-plus').style.opacity = "0.7";
        this.querySelector('.product-item-cart-stepper-minus').style.opacity = "0.7";
        ...
        fetch(`${FoxTheme.routes.cart_change_url}`, {
            ...
            .then((updatedCart) => {
                ...
                if (updatedCart.errors) {
                    console.log("Cart Error : ", updatedCart.errors)
                    this.syncQtyFromCart(window.ShopifyCart.data);
                    return
                }
                ...
                window.ShopifyCart.data = updatedCart;
                document.dispatchEvent(
            ...
            .finally(() => {
                ...
                this.querySelector('.product-item-cart-stepper-plus').style.opacity = "1";
                this.querySelector('.product-item-cart-stepper-minus').style.opacity = "1";
    ...
    updateCartState = (cartJson) => {
        FoxTheme.pubsub.publish(FoxTheme.pubsub.PUB_SUB_EVENTS.cartUpdate, { cart: cartJson });
        console.log("Updating Counter ::: ", cartJson.item_count)
    };
...
customElements.define('card-qty', CardQty);
// ================ CARD QTY BOX END ================
